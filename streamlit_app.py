...
COMPLETION_HISTORY_DELETED_KEYS_PATH = LOG_DIR / "completion_history_deleted_keys.json"
...
COMPLETION_HISTORY_FALLBACK_ROWS = [
    ...
    {
        "교체완료시각": "2026-06-23 20:02:13",
        "설비": "엣지 #3,4",
        "날물명": "AT 날물(후면)",
        "기준값": "90,000m",
        "교체 시점 사용량": "0m",
        "담당자": "김정철",
        "비고": "잔이바리 발생(4호기 at날물교체)",
    },
    {
        "교체완료시각": "2026-06-23 12:40:41",
        "설비": "엣지 #3,4",
        "날물명": "AT 날물(후면)",
        "기준값": "90,000m",
        "교체 시점 사용량": "81,330m",
    ...
    {
        "교체완료시각": "2026-05-29 13:26:15",
        "설비": "엣지 #3,4",
        "날물명": "AT 날물(후면)",
        "기준값": "90,000m",
        "교체 시점 사용량": "81,020.044m",
        "담당자": "김정철",
...
def build_initial_raw_data() -> list[dict[str, Any]]:
    ...
    for machine_config in BORING_MACHINE_CONFIG:
        ...
        for blade_spec in BORING_BLADE_SPECS:
            ...
            if machine_config["line"] == "런닝" and blade_spec["bladeName"] == "Φ8(관통) 날물":
...
def save_completion_history_deleted_keys(deleted_keys: set[str]) -> None:
    ...
    normalized_keys = sorted(str(key).strip() for key in deleted_keys if str(key).strip())

    COMPLETION_HISTORY_DELETED_KEYS_PATH.write_text(
        json.dumps(normalized_keys, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )

    st.session_state.completion_history_deleted_keys = normalized_keys


def get_completion_history_identity(entry: dict[str, Any]) -> str:
...
def normalize_completion_history(history: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for entry in history:
        ...
        if completed_at == "2026-06-16 14:13:52":
        ...
        if (
            ...
            usage_label = "103,207 회"

        if (
            completed_at == "2026-05-19 13:35:31"
            and normalized_machine == "런닝 #22"
            and normalized_blade == "Φ5(관통) 날물"
        ):
        ...
        if (
            ...
            and normalized_machine == "런닝 #22"
            and normalized_blade == "Φ5(관통) 날물"
            and parse_numeric_value(usage_label) == 116995
        ):
        ...
        if (
            ...
            and normalized_machine == "런닝 #21"
            and normalized_blade == "Φ20 날물"
            and parse_numeric_value(usage_label) == 57520
        ):
...
def append_completion_history_store(
    ...
    values = store_worksheet.get_all_values()
...
def get_completion_history_key(entry: dict[str, Any]) -> str:
    ...
    return get_completion_history_identity(row)
